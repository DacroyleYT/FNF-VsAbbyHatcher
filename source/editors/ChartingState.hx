...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		function loadMetadata() {
			...
			var file:String = Paths.json(songName + '/metadata');
			#if sys
			if (#if MODS_ALLOWED FileSystem.exists(Paths.modsJson(songName + '/metadata')) || #end FileSystem.exists(file))
			#else
			if (OpenFlAssets.exists(file))
			#end
			{
				var metaStuff:CoolMetadata = SongMeta.loadFromJson('metadata', songName);
				return metaStuff;
			}
			else return null;
		}
		var metaJson = loadMetadata();
		trace(Paths.formatToSongPath(_song.song));
		if (metaJson == null)
		{
			_metadata = new SongMeta(_song.song, "Unknown", "Unknown", "Unknown", "", "fuzzly-catcher");
			trace("well guys it's returning null :(");
		}
		else
		{
			//_metadata = metaJson;
			_metadata = new SongMeta(metaJson.songName, metaJson.composer, metaJson.ogComposer, metaJson.charter, metaJson.freeplayDesc, metaJson.freeplaySong);//convertToCoolMetadata(metaJson);
			//PlayState.songTitle = _metadata.songName;
			//PlayState.composer = _metadata.composer;
			//PlayState.ogComposer = _metadata.ogComposer;
			//PlayState.charter = _metadata.charter;
		}
	...
	var Metadata_songName:FlxUIInputText;
	var Metadata_composer:FlxUIInputText;
	var Metadata_ogComposer:FlxUIInputText;
	var Metadata_charter:FlxUIInputText;
	var Metadata_freeplayDesc:FlxUIInputText;
	var Metadata_freeplaySong:FlxUIInputText;

	function addMetadataUI():Void
	{
			var tab_group_metadata = new FlxUI(null, UI_box);
			tab_group_metadata.name = "Metadata";
			Metadata_songName = new FlxUIInputText(10, 30, 120, _metadata.songName, 8);
			tab_group_metadata.add(Metadata_songName);
			tab_group_metadata.add(new FlxText(Metadata_songName.x, Metadata_songName.y - 15, 0, 'Song Name:'));
			blockPressWhileTypingOn.push(Metadata_songName);

			Metadata_composer = new FlxUIInputText(10, 70, 120, _metadata.composer, 8);
			tab_group_metadata.add(Metadata_composer);
			tab_group_metadata.add(new FlxText(Metadata_composer.x, Metadata_composer.y - 15, 0, 'Composer:'));
			blockPressWhileTypingOn.push(Metadata_composer);

			Metadata_ogComposer = new FlxUIInputText(10, 110, 120, _metadata.ogComposer, 8);
			tab_group_metadata.add(Metadata_ogComposer);
			tab_group_metadata.add(new FlxText(Metadata_ogComposer.x, Metadata_ogComposer.y - 15, 0, 'Original composer:'));
			blockPressWhileTypingOn.push(Metadata_ogComposer);
			
			Metadata_charter = new FlxUIInputText(10, 150, 120, _metadata.charter, 8);
			tab_group_metadata.add(Metadata_charter);
			tab_group_metadata.add(new FlxText(Metadata_charter.x, Metadata_charter.y - 15, 0, 'Charter (That\'s you!):'));
			blockPressWhileTypingOn.push(Metadata_charter);

			Metadata_freeplayDesc = new FlxUIInputText(10, 190, 240, _metadata.freeplayDesc, 8);
			tab_group_metadata.add(Metadata_freeplayDesc);
			tab_group_metadata.add(new FlxText(Metadata_freeplayDesc.x, Metadata_freeplayDesc.y - 15, 0, 'Description in Freeplay Menu:'));
			blockPressWhileTypingOn.push(Metadata_freeplayDesc);

			Metadata_freeplaySong = new FlxUIInputText(10, 260, 240, _metadata.freeplaySong, 8);
			tab_group_metadata.add(Metadata_freeplaySong);
			tab_group_metadata.add(new FlxText(Metadata_freeplaySong.x, Metadata_freeplaySong.y - 15, 0, 'Song name to use for Freeplay (example: bricks-and-lifts):'));
			blockPressWhileTypingOn.push(Metadata_freeplaySong);
	...
	function addSongUI():Void
	{
		...
		var saveButton:FlxButton = new FlxButton(110, 8, "Save Chart", function()
		{
			saveLevel();
		});
		...
		var saveMetadata:FlxButton = new FlxButton(110, loadAutosaveBtn.y, 'Save Metadata', function ()
		{
			saveMetadata();
		});
		...
		//tab_group_song.add(new FlxText(stepperBPM.x + 100, stepperBPM.y - 15, 0, 'Song Offset:')); // this text has no purpose because it doesnt even have a text box under it
	...
	override function update(elapsed:Float)
	{
		...
		_metadata.songName = Metadata_songName.text;
		_metadata.composer = Metadata_composer.text;
		_metadata.ogComposer = Metadata_ogComposer.text;
		_metadata.charter = Metadata_charter.text;
		_metadata.freeplayDesc = Metadata_freeplayDesc.text;
		_metadata.freeplaySong = Metadata_freeplaySong.text;
	...
	*/
	...
	{
		var json = {
			songName: _metadata.songName,
			composer: _metadata.composer,
			ogComposer: _metadata.ogComposer,
			charter: _metadata.charter,
			freeplayDesc: _metadata.freeplayDesc,
			freeplaySong: _metadata.freeplaySong
		}
