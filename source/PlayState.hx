...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Are you OK?', 0.2], //From 0% to 19%
		['Terrible', 0.4],
		['Bad', 0.5], //From 40% to 49%
		['Bruh', 0.58], //From 50% to 57%
		['THE TEARS ARE ROLLIN DOWN MY FACE', 0.59], //58%
		['Bruh', 0.6], //59%
		['Meh', 0.7], //From 60% to 68%
		['Good', 0.8], //From 70% to 79%
		['Great', 0.9], //From 80% to 89%
		['Fuzztastic!', 1], //From 90% to 99%
		['Splizzacular!!', 1] //The value on this one (is)n't used actually, since Perfect is always "1"
	];

	//metadata stuff lol
	public static var songTitle:String;
	public static var composer:String;
	public static var ogComposer:String;
	public static var charter:String;
	//we dont need freeplay desc here in PlayState.hx
	...
	public static var metadata:CoolMetadata = null;
	...
	//Handles the new epic moderately attractive cam code that i've done
	...
	override public function create()
	{
		...
		// Crappy layering but whatev it works LOL
		if (curStage == 'limo')
			add(limo);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		/*var metadatafile:String = Paths.json(songName + '/metadata');
		#if MODS_ALLOWED
		if (FileSystem.exists(Paths.modsJson(songName + '/metadata')) || FileSystem.exists(metadatafile)) {
		#else
		if (OpenFlAssets.exists(file)) {
		#end
			var metadataData:Array<Dynamic> = SongMeta.loadFromJson('metadata', songName);
			songTitle = metadataData.songName;
			trace(metadataData.songName);
			composer = metadataData.composer;
			trace("Composed by " + metadataData.composer);
			ogComposer = metadataData.ogComposer;
			trace("Originally composed by " + metadataData.ogComposer);
			charter = metadataData.charter;
			trace("Charted by " + metadataData.charter);
		}*/
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					FlxG.sound.playMusic(Paths.music(ClientPrefs.legacyMenu));
	...
	private function cachePopUpScore()
	{
		...
		else
		{
			...
			Paths.image(pixelShitPart1 + "good" + pixelShitPart2);
			Paths.image(pixelShitPart1 + "bad" + pixelShitPart2);
			Paths.image(pixelShitPart1 + "terrible" + pixelShitPart2);
			Paths.image(pixelShitPart1 + "combo" + pixelShitPart2);
		}
