...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (ClientPrefs.legacyMode) {
			bg.loadGraphic(Paths.image('oldmenus/menuBGBlue-mods'));
		} else {
		...
		noModsTxt = new FlxText(0, 0, FlxG.width, "No mods detected!\nMake sure your mod is located in a mod folder inside mods/", 48);
		if(FlxG.random.bool(0.1)) noModsTxt.text = 'No mods detected! GET OUT!!!'; // meanie
		...
		if (FileSystem.exists("modsList.txt")){
			for (folder in Paths.getModDirectories())
			{
				...
				{
					addToModsList([folder, false]); //i like it false by default. -bb //Well, i like it True! -Shadow //False the goat -Dacro
				}
			}
		}
		saveTxt();
		...
		for (mod in mods)
		{
			...
			if(i == curSelected)
			{
				...
				if (mod.restart){//finna make it to where if nothing changed then it won't reset
					descriptionTxt.text += "\n*Moving or toggling this mod will restart the game";
				}
