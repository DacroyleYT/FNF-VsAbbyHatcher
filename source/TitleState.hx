...
class TitleState extends MusicBeatState
{
	...
	override public function create():Void
	{
		...
		#else
		#if SNAPSHOT if (SnapshotState.leftState) { #end
		if (FlxG.save.data.flashing == null && !FlashingState.leftState){
			...
			FlxTransitionableState.skipNextTransIn = true;
			FlxTransitionableState.skipNextTransOut = true;
			MusicBeatState.switchState(new SnapshotState());
		} #end
		#end
	}
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			// HAD TO MODIFY SOME BACKEND STUFF
		...
		switch(easterEgg.toUpperCase())
		{
			...
			//EDIT THIS ONE IF YOU'RE MAKING A SOURCE CODE MOD!!!!
			//EDIT THIS ONE IF YOU'RE MAKING A SOURCE CODE MOD!!!!
				gfDance.frames = Paths.getSparrowAtlas('characters/AbbyBumpin');
				gfDance.animation.addByIndices('danceLeft', 'Abby Left dance', [0, 1, 2, 3, 4], "", 16, false);
				gfDance.animation.addByIndices('danceRight', 'Abby Right dance', [0, 1, 2, 3, 4], "", 16, false);
		}
	...
	override function update(elapsed:Float)
	{
		...
		if (initialized && !transitioning && skippedIntro)
		{
			...
			if(pressedC) // load crash handler for debug purposes
				{
					titleText.color = FlxColor.WHITE;
					titleText.alpha = 1;
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 2:
					#if PSYCH_WATERMARKS
					createCoolText(['Made by'], 15);
					#else
					createCoolText(['DacroyleYT', 'RayoMC95']);
					#end
				// credTextShit.visible = true;
				case 4:
					#if PSYCH_WATERMARKS
					addMoreText('DacroyleYT', 15);
					addMoreText('RayoMC95', 15);
					#else
					addMoreText('present');
					#end
				// credTextShit.text += '\npresent...';
				...
				// credTextShit.text += '\nNight';
				case 16:
					addMoreText('Hatcher'); // credTextShit.text += '\nFunkin';
		...
		if (!skippedIntro)
		{
			if (playJingle) //Ignore deez
			{
				...
				switch(easteregg)
				{
					...
					default: //Go back to normal ugly ahh boring GF
						remove(ngSpr);
						remove(credGroup);
