...
class WeekData {
	...
	public static function reloadWeekFiles(isStoryMode:Null<Bool> = false)
	{
		...
		var sexList:Array<String> = #if DEMO_BUILD CoolUtil.coolTextFile(Paths.getPreloadPath('demoWeeks/weekList.txt')); #else CoolUtil.coolTextFile(Paths.getPreloadPath('weeks/weekList.txt')); #end
		var weekDir = #if DEMO_BUILD 'demoWeeks/'; #else 'weeks/'; #end
		if (ClientPrefs.legacyMode) {
			weekDir = 'legacyWeeks/';
		}
		for (i in 0...sexList.length) {
			for (j in 0...directories.length) {
				var fileToCheck:String = directories[j] + weekDir + sexList[i] + '.json';
		...
		for (i in 0...directories.length) {
			var directory:String = directories[i] + weekDir;
